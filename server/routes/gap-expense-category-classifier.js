...
// Route: /api/gap-expense-category-classifier
// Feature: Expense Category Classifier
// Description: (auto-categorize invoices via ML)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Expense Category Classifier. (auto-categorize invoices via ML)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Expense Category Classifier\nDescription: (auto-categorize invoices via ML)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['expense-category-classifier', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'expense-category-classifier', title: 'Expense Category Classifier', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'expense-category-classifier', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['expense-category-classifier']);
      return res.json({ slug: 'expense-category-classifier', items: r.rows || [] });
    }
    return res.json({ slug: 'expense-category-classifier', items: [] });
  } catch (e) {
    return res.json({ slug: 'expense-category-classifier', items: [], error: (e && e.message) || 'Server error' });
  }
});
