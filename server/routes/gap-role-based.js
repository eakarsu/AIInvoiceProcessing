...
// Route: /api/gap-role-based
// Feature: Role Based
// Description: Role-based access controls (RBAC matrix)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Role Based. Role-based access controls (RBAC matrix)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'role-based', title: 'Role Based', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'role-based', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['role-based']);
      return res.json({ slug: 'role-based', items: r.rows || [] });
    }
    return res.json({ slug: 'role-based', items: [] });
  } catch (e) {
    return res.json({ slug: 'role-based', items: [], error: (e && e.message) || 'Server error' });
  }
});
