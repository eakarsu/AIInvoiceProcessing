...
// Route: /api/gap-cash-flow-forecast
// Feature: Cash Flow Forecast
// Description: (combine AR/AP timing for liquidity projection)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Cash Flow Forecast. (combine AR/AP timing for liquidity projection)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Cash Flow Forecast\nDescription: (combine AR/AP timing for liquidity projection)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['cash-flow-forecast', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'cash-flow-forecast', title: 'Cash Flow Forecast', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'cash-flow-forecast', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['cash-flow-forecast']);
      return res.json({ slug: 'cash-flow-forecast', items: r.rows || [] });
    }
    return res.json({ slug: 'cash-flow-forecast', items: [] });
  } catch (e) {
    return res.json({ slug: 'cash-flow-forecast', items: [], error: (e && e.message) || 'Server error' });
  }
});
