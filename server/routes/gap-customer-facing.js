...
// Route: /api/gap-customer-facing
// Feature: Customer Facing
// Description: Customer-facing supplier portal
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Customer Facing. Customer-facing supplier portal' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Customer Facing\nDescription: Customer-facing supplier portal\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['customer-facing', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'customer-facing', title: 'Customer Facing', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'customer-facing', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['customer-facing']);
      return res.json({ slug: 'customer-facing', items: r.rows || [] });
    }
    return res.json({ slug: 'customer-facing', items: [] });
  } catch (e) {
    return res.json({ slug: 'customer-facing', items: [], error: (e && e.message) || 'Server error' });
  }
});
