// Auto-generated feature route: erp-webhook
// Domain: ERP integration engineer — Capability: emitting ERP webhook payloads for posting integrations
// Mounted under: /api/erp-webhook
...
// POST /api/erp-webhook/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a ERP integration engineer. Your task is emitting ERP webhook payloads for posting integrations. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/erp-webhook/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/erp-webhook/:id — fetch one
...
// DELETE /api/erp-webhook/:id — discard a run
