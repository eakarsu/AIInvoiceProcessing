...
// Route: /api/gap-notification-alerting
// Feature: Notification Alerting
// Description: Notification/alerting subsystem
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Notification Alerting. Notification/alerting subsystem' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Notification Alerting\nDescription: Notification/alerting subsystem\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['notification-alerting', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'notification-alerting', title: 'Notification Alerting', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'notification-alerting', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['notification-alerting']);
      return res.json({ slug: 'notification-alerting', items: r.rows || [] });
    }
    return res.json({ slug: 'notification-alerting', items: [] });
  } catch (e) {
    return res.json({ slug: 'notification-alerting', items: [], error: (e && e.message) || 'Server error' });
  }
});
