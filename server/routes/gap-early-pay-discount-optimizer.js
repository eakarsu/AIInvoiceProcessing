...
// Route: /api/gap-early-pay-discount-optimizer
// Feature: Early Pay Discount Optimizer
// Description: (negotiate discount vs. cash carrying cost)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Early Pay Discount Optimizer. (negotiate discount vs. cash carrying cost)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Early Pay Discount Optimizer\nDescription: (negotiate discount vs. cash carrying cost)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['early-pay-discount-optimizer', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'early-pay-discount-optimizer', title: 'Early Pay Discount Optimizer', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'early-pay-discount-optimizer', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['early-pay-discount-optimizer']);
      return res.json({ slug: 'early-pay-discount-optimizer', items: r.rows || [] });
    }
    return res.json({ slug: 'early-pay-discount-optimizer', items: [] });
  } catch (e) {
    return res.json({ slug: 'early-pay-discount-optimizer', items: [], error: (e && e.message) || 'Server error' });
  }
});
