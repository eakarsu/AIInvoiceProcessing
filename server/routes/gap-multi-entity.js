...
// Route: /api/gap-multi-entity
// Feature: Multi Entity
// Description: Multi-entity / subsidiary consolidation
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Multi Entity. Multi-entity / subsidiary consolidation' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'multi-entity', title: 'Multi Entity', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'multi-entity', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['multi-entity']);
      return res.json({ slug: 'multi-entity', items: r.rows || [] });
    }
    return res.json({ slug: 'multi-entity', items: [] });
  } catch (e) {
    return res.json({ slug: 'multi-entity', items: [], error: (e && e.message) || 'Server error' });
  }
});
