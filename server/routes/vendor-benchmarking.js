// Auto-generated feature route: vendor-benchmarking
// Domain: AP benchmarking analyst — Capability: benchmarking vendor terms against the peer group
// Mounted under: /api/vendor-benchmarking
...
// POST /api/vendor-benchmarking/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a AP benchmarking analyst. Your task is benchmarking vendor terms against the peer group. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/vendor-benchmarking/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/vendor-benchmarking/:id — fetch one
...
// DELETE /api/vendor-benchmarking/:id — discard a run
