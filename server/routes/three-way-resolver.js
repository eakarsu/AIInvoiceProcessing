// Auto-generated feature route: three-way-resolver
// Domain: AP reconciliation agent — Capability: resolving 3-way match discrepancies and proposing journal corrections
// Mounted under: /api/three-way-resolver
...
// POST /api/three-way-resolver/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a AP reconciliation agent. Your task is resolving 3-way match discrepancies and proposing journal corrections. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/three-way-resolver/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/three-way-resolver/:id — fetch one
...
// DELETE /api/three-way-resolver/:id — discard a run
